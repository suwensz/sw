...
import { COUNTRY_SOCIAL_MAP, SOCIAL_APP_MAP, productMarket, countryInfo } from '@/mock/socialData'
...
// 客户国家主流社交软件（优先匹配国家，否则商品主销市场）
const contactApps = computed(() => {
  if (!product.value) return []
  const matched = COUNTRY_SOCIAL_MAP.find((x) => x.code === social.state.matchCountry)
  const market = matched || productMarket(product.value.id)
  return market.apps.slice(0, 3).map((id) => SOCIAL_APP_MAP[id])
})
const contactCountry = computed(() => {
  if (!product.value) return null
  const matched = COUNTRY_SOCIAL_MAP.find((x) => x.code === social.state.matchCountry)
  return matched || productMarket(product.value.id)
})

function chatNow(appId: SocialAppId) {
  if (product.value) social.openChat(appId, product.value)
}

const slug = computed(() => route.params.slug as string)
const product = computed(() => {
  return mockProducts.find((p) => p.slug === slug.value)
    || sellerStore.products.find((p) => p.slug === slug.value)
})

const quantity = ref(1)
const activeTab = ref('description')

// 分类 i18n 键：health_tonic → categoryHealthTonic
function catLabel(cat: string): string {
  const key = 'category' + cat.split('_').map((s) => s.charAt(0).toUpperCase() + s.slice(1)).join('')
  return t('shop.' + key)
}
    ...
    <div class="detail-layout">
      ...
      <div class="detail-info">
        ...
        <div class="product-rating">
          ...
          <span class="sales-info">{{ t('shop.salesLabel') }} {{ localizeNumber(product.sales, locale) }}</span>
        ...
        <div class="product-meta-row">
          ...
          <el-tag size="small" effect="plain">{{ catLabel(product.category) }}</el-tag>
        </div>
        ...
        <!-- 社交软件咨询（按客户国家主流软件同步） -->
        <div v-if="contactCountry" class="social-contact">
          <span class="contact-label">
            <el-icon><ChatDotRound /></el-icon>
            {{ t('social.contactVia') }}
            <span class="contact-country">{{ contactCountry.flag }} {{ tText(contactCountry.name, locale as any) }}</span>
          </span>
          <div class="contact-apps">
            <button
              v-for="a in contactApps"
              :key="a.id"
              class="contact-btn"
              :style="{ color: a.color, borderColor: a.color + '66', background: a.color + '0f' }"
              @click="chatNow(a.id)"
            >
              <span class="contact-dot" :style="{ background: a.color }"></span>
              {{ tText(a.name, locale as any) }}
            </button>
          </div>
        </div>
        ...
        <div class="store-info">
          ...
          <div class="store-detail">
            <p class="store-name">{{ t('nav.shop') }} · SuhengOS</p>
            <div class="store-meta">
              <span class="store-badge">{{ t('common.freeShipping') }}</span>
              <span class="store-rating">⭐ 4.8</span>
              <span class="store- followers">100K+</span>
            </div>
          </div>
        </div>
