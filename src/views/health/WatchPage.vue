...
// 关系显示：预设关系走 i18n，自定义关系原样显示
const KNOWN_RELATIONSHIPS = ['self', 'parent', 'spouse', 'child', 'sibling', 'other']
function relationshipLabel(r: string): string {
  return KNOWN_RELATIONSHIPS.includes(r) ? t(`family.relationships.${r}`) : r
}

function addDevice() {
  const input = newDevice.value.memberId.trim()
  if (!input || !newDevice.value.name) return
  // 绑定家人支持可编辑：输入的可能是已有家人 id、家人姓名，或全新姓名
  let memberId = input
  const byId = health.familyMembers.find((m) => m.id === input)
  if (!byId) {
    const byName = health.familyMembers.find((m) => m.name === input)
    if (byName) {
      memberId = byName.id
    } else {
      // 直接输入新姓名 → 自动创建家人（默认资料，可稍后在家人管理中完善）
      const created = health.addMember({
        name: input,
        relationship: 'other',
        gender: 'other',
        calendarType: 'solar',
  ...
  health.addWatch({ ...newDevice.value, memberId })
    ...
    <el-dialog v-model="showAddDialog" :title="t('watch.addDevice')" width="460px">
      <el-form label-position="top">
        <el-form-item :label="t('watch.bindMember')">
          <el-select
            v-model="newDevice.memberId"
            style="width: 100%"
            filterable
            allow-create
            default-first-option
            :placeholder="t('watch.bindMemberPlaceholder')"
          >
            <el-option
          ...
          <div class="form-hint">{{ t('watch.bindMemberHint') }}</div>
        </el-form-item>
...
.form-hint { font-size: 12px; color: var(--color-text-secondary); margin-top: 4px; line-height: 1.5; }
